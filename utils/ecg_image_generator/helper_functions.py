...
def read_config_file(config_file):
    ...
    if Path(config_file).suffix == ".json":
        with open(config_file, "r") as f:
            configs = json.load(f)
    elif Path(config_file).suffix in [".yaml", ".yml"]:
        with open(config_file, "r") as f:
...
def convert_bounding_boxes_to_dict(
    lead_bboxes, text_bboxes, labels, startTimeList=None, endTimeList=None, plotted_pixels_dict=None
):
    leads_ds = []
    ...
    for i in range(len(labels)):
        ...
        current_lead_ds["plotted_pixels"] = [
            [plotted_pixels_dict[i][j][0], plotted_pixels_dict[i][j][1]] for j in range(len(plotted_pixels_dict[i]))
        ]
