...
import numpy as np
from constants import save_img_ext
...
def ecg_plot(
    ...
    for i in np.arange(len(lead_index)):
        ...
        if show_lead_name:
            t1 = ax.text(x_offset + x_gap + dc_offset, y_offset - lead_name_offset - 0.2, leadName, fontsize=lead_fontsize)
    ...
    if full_mode != "None":
        current_lead_ds = dict()
        if show_lead_name:
            t1 = ax.text(x_gap + dc_offset, row_height / 2 - lead_name_offset, full_mode, fontsize=lead_fontsize)
    ...
    if show_grid:
        ...
        if store_configs == 2:
            json_dict["grid_line_color_major"] = [round(x * 255.0, 2) for x in color_major]
            json_dict["grid_line_color_minor"] = [round(x * 255.0, 2) for x in color_minor]
